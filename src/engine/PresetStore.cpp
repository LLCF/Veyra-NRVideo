...
bool PresetStore::parse(const std::string& data,std::vector<UserPreset>& out,std::wstring& def){
    ...
    if(!(in>>magic>>version)||magic!="VEYRA_PRESETS"||(version<1||version>18)||!(in>>std::quoted(d)>>count)||count>64)return false;def=wide(d);if(!d.empty()&&def.empty())return false;
    for(size_t i=0;i<count;++i){UserPreset p;std::string n;int policy,flow,content,nr,sr;auto& s=p.settings;auto& m=s.model;auto& r=s.residual;
        ...
        if(version>=18){std::string lut;if(!readColorSettings(in,s.color,lut))return false;if(!lut.empty()&&!s.color.setLutName(wide(lut)))return false;}
