...
std::string PresetStore::serialize()const{
    std::ostringstream o;o.imbue(std::locale::classic());o<<std::setprecision(std::numeric_limits<float>::max_digits10)<<"VEYRA_PRESETS 20\n"<<std::quoted(utf8(default_))<<' '<<entries_.size()<<'\n';
...
bool PresetStore::parse(const std::string& data,std::vector<UserPreset>& out,std::wstring& def){
    ...
    if(!(in>>magic>>version)||magic!="VEYRA_PRESETS"||(version<1||version>20)||!(in>>std::quoted(d)>>count)||count>64)return false;def=wide(d);if(!d.empty()&&def.empty())return false;
    for(size_t i=0;i<count;++i){UserPreset p;std::string n;int policy,flow,content,nr,sr;auto& s=p.settings;auto& m=s.model;auto& r=s.residual;
        ...
        if(version>=20){int enabled;if(!(in>>enabled>>s.videoHdr.contrast>>s.videoHdr.saturation>>s.videoHdr.middleGray>>s.videoHdr.peakNits)||enabled<0||enabled>1)return false;s.videoHdr.enabled=enabled!=0;}
