...
std::string PresetStore::serialize()const{
    std::ostringstream o;o.imbue(std::locale::classic());o<<std::setprecision(std::numeric_limits<float>::max_digits10)<<"VEYRA_PRESETS 14\n"<<std::quoted(utf8(default_))<<' '<<entries_.size()<<'\n';
...
bool PresetStore::parse(const std::string& data,std::vector<UserPreset>& out,std::wstring& def){
    ...
    if(!(in>>magic>>version)||magic!="VEYRA_PRESETS"||(version<1||version>14)||!(in>>std::quoted(d)>>count)||count>64)return false;def=wide(d);if(!d.empty()&&def.empty())return false;
