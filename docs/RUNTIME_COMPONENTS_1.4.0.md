# Veyra 1.4.0 Runtime Components
...
1.4.0 与 1.3.2 的**运行组件身份完全一致**（下面这张表逐项沿用）。唯一的功能变化是
**AMD FSR 补帧不再出现在界面上**——组件仍然随包分发，引擎后端保留。
...
is an experimental choice, not a claim of verified RTX30 compatibility. Fresh installations leave NR, SR,
and internal FG off.
