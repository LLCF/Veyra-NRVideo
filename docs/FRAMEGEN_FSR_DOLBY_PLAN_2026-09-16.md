...
> **实施结果（2026-09-16 凌晨更新，分支 `codex/framegen-fsr-dolby-20260916`，未合并 main）**
>
> | 工作流 | 结果 | 证据 |
> | --- | --- | --- |
> | A XeSS MFG（2X/3X/4X） | **已完成**（先前提交 `4124a9d`） | 4X 实跑 1677 生成/563 真实，退出时 5/5 回滚 |
> | A-2 XeSS 节奏 hook | **未移植**（基础设施 `ThunkHook` 已就位） | 状态文档 §二 |
> | B DLSS 6X | **已完成**（`35dd632`） | 6X 实跑 2215 生成/445 真实 |
> | C-2 40 系 Ada 解锁 | **调研完成、未实施**（不交付半成品黑帧补丁） | `docs/WORKLOG.md` 2026-09-16 条目 |
> | D 30 系原生 2X | **未开始**（需另一份运行库身份授权） | — |
> | E AMD FSR 补帧 | **已完成并实测 2X**（`27656a6`…）：真实运动、节奏、4K 几何、设置重建全部有据 | `docs/FSR_FRAMEGEN_INTEGRATION_2026-09-16.md` |
> | F FSR 超分 | **已接入产品并实测（3.1.x，2X 档位之外的独立 SR 档）**：内容正确、非 NVIDIA 形状可跑；相对画质略软已如实记录 | `docs/FSR_UPSCALING_PLAN_2026-09-16.md` |
> | G-1 杜比位流探测 | **已完成**（本机采集卡不提供位流） | 状态文档 §一.3 |
> | G-2 杜比直通/解码 | **未开始**（缺支持位流的硬件，无法验证） | 状态文档 §二 |
>
> 逐项验收清单与最终构建/测试结果见 `docs/FRAMEGEN_FSR_DOLBY_STATUS_2026-09-16.md`。
