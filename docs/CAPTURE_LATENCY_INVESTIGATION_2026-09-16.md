...
采集卡 `USB3 Video`（VID_345F，用户那台圆刚类设备）当前**被其他程序独占**：
`capture:0:0:-1:0`（YUY2 1080p60）与 `capture:0:24:-1:0`（MJPEG 1080p60）都在 `Run` 阶段
返回 `0x800705AA`（ERROR_NO_SYSTEM_RESOURCES）——设备枚举正常但图起不来，OBS 正在运行
（它的虚拟摄像头在枚举里可见）。**没有强杀 OBS**，所以实卡 A/B 留到用户释放设备后再测（§6）。
