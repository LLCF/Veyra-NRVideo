...
| 30 系原生 2X（D） | **决策：搁置（2026-09-16 复查后维持）** | `sdli1995/dlssg_for_sm86` 实测无源码、无 LICENSE 文件（README 自述 GPLv3，GitHub 侧栏标签由此推断），本体是伪装 DLL 的代理加载器；dashdogy v1.3.3 的 30 系实验路径依赖**不在公开源码树里的 SM86 kernel cache**（BUILD.md 原话"source checkout alone cannot reproduce the DLL"），无法合法复现。30 系用户走 Veyra 自带 FSR 帧生成（E）。详见 [30 系补帧决策](DLSSG_30SERIES_DECISION_2026-09-16.md)。**未验证**：FSR 补帧在 RTX 30 实机上能否运行 |
