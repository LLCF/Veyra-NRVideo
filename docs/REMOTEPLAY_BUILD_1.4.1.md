...
The 1.4.1 portable package enables `VEYRA_ENABLE_REMOTEPLAY`. The default build without this option does
not include PS5 Remote Play. This package is built from `main` (the isolated colour/AverMedia/export
branches were merged before the build); the `v1.4.1` tag will be created when the build is released.

Application source: https://github.com/Likely7/Veyra-NRVideo (tag `v1.4.1` at release time)
...
`Veyra-1.4.1-RemotePlay-source.zip` supplies the clean pinned Chiaki tree and recursive submodule files,
