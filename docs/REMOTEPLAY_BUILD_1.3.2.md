...
The 1.3.2 portable package enables `VEYRA_ENABLE_REMOTEPLAY`. The default build without this option does not include PS5 Remote Play. This beta package is built from the isolated branch `codex/capture-decode-latency-20260916` (not merged, not pushed); the `v1.3.2` tag will be created when the build is released.

Application source: https://github.com/Likely7/Veyra-NRVideo (tag `v1.3.2` at release time)
